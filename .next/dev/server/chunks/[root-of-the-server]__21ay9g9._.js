...
"[project]/app/lib/social.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function analyzeSocial(businessName, city) {
    const apiKey = process.env.SERPAPI_KEY;
    if (!apiKey) {
        console.error("SERPAPI_KEY non configurata");
        return emptyResult();
    ...
    try {
        ...
        return {
            ...
            instagram: {
                found: instagram.found,
                url: instagram.url,
                verified: instagram.verified ?? false
            },
            facebook: {
                found: facebook.found,
                url: facebook.url
            },
            tiktok: {
                found: tiktok.found,
...
function checkNameConsistency(businessName, titles) {
    const normalizedBusiness = businessName.toLowerCase().replace(/[^a-z0-9]/g, "");
    const foundTitles = titles.filter(Boolean);
...
function findInResults(data, domain) {
    // Cerca nei risultati organici
    if (data.organic_results) {
        const found = data.organic_results.find((r)=>r.link?.includes(domain));
        if (found) return found;
    }
    // Cerca nel knowledge graph (social links)
    if (data.knowledge_graph?.profiles) {
        const profile = data.knowledge_graph.profiles.find((p)=>p.link?.includes(domain));
        if (profile) return {
            link: profile.link,
            title: profile.name || ""
        };
    ...
    if (data.knowledge_graph?.social_links) {
        const link = data.knowledge_graph.social_links.find((l)=>l.link?.includes(domain));
        if (link) return {
            link: link.link,
            title: ""
        };
...
async function searchSocial(businessName, city, apiKey, platform, domain) {
    try {
        ...
        const response = await fetch(url);
        const data = await response.json();
        console.log(`${platform} primi 3 link:`, data?.organic_results?.slice(0, 3).map((r)=>r.link));
...
async function searchTripAdvisor(businessName, city, apiKey) {
    try {
...
var __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$lib$2f$social$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/app/lib/social.ts [app-route] (ecmascript)");
;
;
;
;
...
async function POST(request) {
    try {
        ...
        // 5 — Social
        const socialResult = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$lib$2f$social$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["analyzeSocial"])(businessName, city);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__21ay9g9._.js.map